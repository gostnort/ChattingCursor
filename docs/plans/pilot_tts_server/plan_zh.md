...
**更正**：`docs/plans/pilot_tts_installation/plan.md` §4.1 仍显示 `upstream/api.py`；实际 `run.bat` 已使用 `server/tts_server.py`。本计划 Phase 1 更新该过时引用。
...
| 安装计划中过时 `api.py` | P0 | `pilot_tts_installation/plan.md:314` | 文档漂移 |
...
上游函数（审计预期签名；实现时对照克隆的 `upstream/demo.py` 验证）：
...
2. `os.chdir(upstream_dir())` —— **P2 候选移除项**（绝对路径测试后）

在每个惰性 import 前添加中文块注释，引用章程例外（上游不可 pip 安装）。
